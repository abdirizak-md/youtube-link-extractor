...
export default function YouTubePlaylistExtractor() {
  ...
  const extractLinks = async () => {
    try {
      ...
      do {
        ...
        if (data.items) {
          allVideos = [
            ...
            ...data.items.map(
              (item) =>
                `https://www.youtube.com/watch?v=$${item.snippet.resourceId.videoId}&format=${format}`
            ),
          ];
        }
      ...
      setVideoLinks(allVideos);
    } catch (error) {
      alert("Failed to fetch playlist videos. Please check your API key and URL.");
    }
  };
  ...
  const copyAllToClipboard = () => {
    const allLinks = videoLinks.join("\n");
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto p-6 max-w-2xl bg-white shadow-lg rounded-lg mt-6">
        ...
        />
        <div className="mb-4 flex flex-col sm:flex-row justify-center items-center gap-4 border rounded mb-10">
        ...
        <button onClick={extractLinks} className="w-full mb-4 bg-blue-600 text-white p-2 rounded">
          Extract Links
        </button>

        <div>
          {videoLinks.map((link, index) => (
            <div key={index} className="flex justify-between items-center mb-2 p-2 border rounded flex-wrap">
              <a
                href={link}
                target="_blank"
                rel="noopener noreferrer"
                className="text-blue-600 break-all mr-2"
              >
                {link}
              </a>
              <button onClick={() => copyToClipboard(link, index)} className="flex items-center">
        ...
        <div className="mt-8 shadow p-4 bg-blue-500 rounded text-white">
      ...
      <footer className="bg-gray-200 text-center text-sm text-gray-600 p-4 mt-6">
