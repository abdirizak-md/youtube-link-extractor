...
export const App = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${darkMode ? "bg-gray-900 text-white" : "bg-white text-black"}`}>
      <header className={`p-4 shadow-md ${darkMode ? "bg-gray-800" : "bg-white"}`}>
        <div className="container mx-auto flex justify-between items-center">
          
          <svg xmlns="http://www.w3.org/2000/svg" id="yt-ringo2-svg_yt6" width="93" height="20" viewBox="0 0 93 20" focusable="false" aria-hidden="true" className='cursor-pointer'> <g>
              <path d="M14.4848 20C14.4848 20 23.5695 20 25.8229 19.4C27.0917 19.06 28.0459 18.08 28.3808 16.87C29 14.65 29 9.98 29 9.98C29 9.98 29 5.34 28.3808 3.14C28.0459 1.9 27.0917 0.94 25.8229 0.61C23.5695 0 14.4848 0 14.4848 0C14.4848 0 5.42037 0 3.17711 0.61C1.9286 0.94 0.954148 1.9 0.59888 3.14C0 5.34 0 9.98 0 9.98C0 9.98 0 14.65 0.59888 16.87C0.954148 18.08 1.9286 19.06 3.17711 19.4C5.42037 20 14.4848 20 14.4848 20Z" fill="#FF0033"></path>
              <path d="M19 10L11.5 5.75V14.25L19 10Z" fill="white"></path> </g>
          ...
          <div className="flex gap-2 items-center">
                ...
                <Logout onLogout={setIsLoggedIn} />
                    ...
                    <button onClick={() => setDarkMode(!darkMode)} className="text-xl ml-10 rounded border cursor-pointer">
        ...
        <div>
            {authView === "login" ? (
            <Login onLogin={setIsLoggedIn} darkMode={darkMode} />
            ) : (
            <SignUp onAuth={setAuthView} darkMode={darkMode} />
            )}
        </div>
            )}

    {isLoggedIn && (
      <main className={`flex-grow container mx-auto p-6 max-w-2xl shadow-lg rounded-lg mt-6 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
        ...
        <div className={`mb-4 flex flex-col sm:flex-row justify-center items-center gap-4 border rounded ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}>
          <label className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Format:</label>
          <select value={format}
            onChange={(e) => setFormat(e.target.value)}
            className={`p-2 rounded ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-black border border-gray-300'}`} >
        ...
        <button  onClick={extractLinks}
            className="w-full mb-4 bg-blue-600 text-white p-2 rounded hover:bg-blue-700 transition cursor-pointer"  >Extract Links</button>
        <div>
            {videoLinks.map((link, index) => (
                <div key={index}
                    className={`flex justify-between items-center mb-2 p-2 border rounded flex-wrap ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}>
                    <a href={link}
                    target="_blank"
        ...
        <div className={`mt-8 shadow p-4 rounded ${darkMode ? 'bg-blue-800 text-white' : 'bg-blue-100 text-black'}`}>
          <h3 className="text-xl font-semibold mb-4">How to Extract YouTube Playlists?</h3>
          <p className="mb-4">
            Extract playlists from YouTube in just one simple step. Here's how:
          </p>
          <ol className="list-inside list-disc pl-4">
            <li className="mb-2">
              <strong>Copy and paste playlist link</strong>
              <br />
              Open YouTube and copy the link of the playlist that you want to extract. Next, paste it in the input field above.
            </li>
          </ol>
        </div>
      </main>
    ...
    <footer className={`text-center text-sm p-4 mt-6 ${darkMode ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
      <div className="container mx-auto">
        <a href="/terms" className="hover:underline mx-2">Terms & Conditions</a>|
        <a href="/privacyPolicy" className="hover:underline mx-2">Privacy Policy</a>
      </div>
    </footer>
  </div>
);
